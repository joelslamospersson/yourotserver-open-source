...
if [[ ! -f "${USER_HOME}/.l_canary.1.2.done" ]]; then
  ...
  log "[1.2/7] skipped (already done)"
fi

# —————————————————————————————
# Make 1 GB swap so builds don’t OOM on small VMs
SWAPFILE=/swapfile
...
# Throttle vcpkg and CMake to one job each, to stay under RAM
...
if [[ ! -d "${VCPKG_DIR}" ]]; then
  ...
  log "[4/7] checking out vcpkg ${LATEST_TAG}"
  ...
  sudo -u "${APP_USER}" env \
    ...
    ./bootstrap-vcpkg.sh --use-system-libs
else
  log "[4/7] skipped (vcpkg already present)"
fi
